module EmailSpec
  module Matchers
    class EmailMatcher
      def address_array
        if @user.email.perform_deliveries
          Array(yield)
        else
          []
        end
    ...
    class ReplyTo
      ...
      def failure_message
        "expected #{@user.email.inspect} to reply to #{@expected_reply_to.address.inspect}, but it replied to #{@actual_reply_to.inspect}"
      end

      def negative_failure_message
        "expected #{@user.email.inspect} not to deliver to #{@expected_reply_to.address.inspect}, but it did"
      end
    end
    ...
    class DeliverTo < EmailMatcher
      ...
      def failure_message
        "expected #{@user.email.inspect} to deliver to #{@expected_recipients.inspect}, but it delivered to #{@actual_recipients.inspect}"
      end

      def negative_failure_message
        "expected #{@user.email.inspect} not to deliver to #{@expected_recipients.inspect}, but it did"
      end
    end
    ...
    class DeliverFrom < EmailMatcher
      ...
      def failure_message
        %(expected #{@user.email.inspect} to deliver from "#{@expected_sender.to_s}", but it delivered from "#{@actual_sender.to_s}")
      end

      def negative_failure_message
        %(expected #{@user.email.inspect} not to deliver from "#{@expected_sender.to_s}", but it did)
      end
    end
    ...
    class BccTo < EmailMatcher
      ...
      def failure_message
        "expected #{@user.email.inspect} to bcc to #{@expected_email_addresses.inspect}, but it was bcc'd to #{@actual_recipients.inspect}"
      end

      def negative_failure_message
        "expected #{@user.email.inspect} not to bcc to #{@expected_email_addresses.inspect}, but it did"
      end
    end
    ...
    class CcTo < EmailMatcher
      ...
      def failure_message
        "expected #{@user.email.inspect} to cc to #{@expected_email_addresses.inspect}, but it was cc'd to #{@actual_recipients.inspect}"
      end

      def negative_failure_message
        "expected #{@user.email.inspect} not to cc to #{@expected_email_addresses.inspect}, but it did"
      end
    end
